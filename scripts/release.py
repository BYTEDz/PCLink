...
def update_changelog(version):
    ...
    if f"## [{version}]" in content:
        print_color(
            f"Warning: Version {version} already in changelog. Skipping.",
            Colors.WARNING,
        )
        return
    ...
    if release_notes_file.exists():
        print_color(
            f"✓ Found {release_notes_file}, using it for release notes.", Colors.OKGREEN
        )
        ...
        print_color(
            f"Error: {release_notes_file} not found. Please create it with your release notes.",
            Colors.FAIL,
        )
        sys.exit(1)
    ...
    try:
        ...
        print_color(
            f"Warning: Could not empty {release_notes_file}: {e}", Colors.WARNING
        )
...
def main():
    ...
    parser.add_argument(
        "--version",
        help="Specify version number directly (e.g., 1.2.0 or 1.2.0-beta.1).",
    )
    parser.add_argument(
        "--beta", action="store_true", help="Mark this as a beta/pre-release."
    )
    ...
    if not args.force:
        ...
        if run_command(["git", "status", "--porcelain"], capture_output=True).stdout:
            ...
            print_color("✓ Git working directory is clean.", Colors.OKGREEN)
    ...
    if not version:
        while True:
            ...
            if re.match(r"^\d+\.\d+\.\d+(-[\w.-]+)?$", new_version):
                ...
                print_color(
                    "Invalid format. Use Semantic Versioning (X.Y.Z or X.Y.Z-beta.1).",
                    Colors.FAIL,
                )
    else:
    ...
    print_color(
        f"✓ Version set to {version} (tag: {tag}, type: {release_type})", Colors.OKGREEN
    )
    ...
    if not release_notes_file.exists():
        ...
        print_color(
            "Please create RELEASE_NOTES.md with your release notes.", Colors.FAIL
        )
        sys.exit(1)
    ...
    print(
        f"  1. Commit changes with message: {Colors.OKCYAN}Bump version to {version}{Colors.ENDC}"
    )
    print(f"  2. Create Git tag: {Colors.OKCYAN}{tag}{Colors.ENDC}")
    print("  3. Push commit and tag to 'origin'.")
    print(
        f"  4. Create GitHub release ({Colors.OKCYAN}{'pre-release/beta' if is_beta else 'latest stable'}{Colors.ENDC})"
    )
    ...
    if is_beta:
        ...
        print_color(
            "✓ Created stable release on GitHub (marked as latest).", Colors.OKGREEN
        )
    print(
        "The GitHub Actions workflow will now build and publish the official release artifacts."
    )
