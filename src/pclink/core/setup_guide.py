...
class PortChecker:
    ...
    def _check_port(self, port: int) -> Dict[str, any]:
        ...
        result = {"available": False, "error": None, "process": None, "can_fix": False}
        ...
        try:
            ...
            try:
                ...
                log.warning(
                    f"Could not identify process using port {port}: {find_error}"
                )
    ...
    def _check_firewall_status(self) -> Dict[str, any]:
        ...
        result = {"enabled": False, "can_configure": False, "profiles": {}}
...
def show_setup_guide(parent=None) -> bool:
    ...
    # Find available port
    available_ports = [
        port for port, info in ports.items() if info.get("available", False)
    ]
