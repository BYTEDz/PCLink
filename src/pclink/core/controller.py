    ...
    def _run_server(self):
        ...
        try:
            ...
            uvicorn_config = {
                ...
                "h11_max_incomplete_event_size": 64
                * 1024
                * 1024,  # 64MB for large uploads
            }
            ...
            if result.returncode == 0:
                ...
                if result.returncode == 0 and "Status: active" in result.stdout:
                    if "38099/udp" not in result.stdout:
                        ...
                        subprocess.run(
                            ["ufw", "allow", "38099/udp"], capture_output=True
                        )
                    if "38080" not in result.stdout:
                        log.info("Adding UFW rule for PCLink API...")
                        subprocess.run(
                            ["ufw", "allow", "38080:38090/tcp"], capture_output=True
                        )
        except Exception as e:
        ...
        try:
            ...
            try:
                ...
                if "netdev" not in user_groups:
                    log.info(
                        "User needs to be added to netdev group for better network access"
                    )
            except KeyError:
                pass
        except Exception as e:
