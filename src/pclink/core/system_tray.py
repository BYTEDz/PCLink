...
class SystemTrayManager:
    ...
    def _check_linux_tray_support(self):
        ...
        if LINUX_NATIVE_TRAY_AVAILABLE:
            ...
            log.warning(
                f"Native Linux AppIndicator support not found: {LINUX_TRAY_ERROR}"
            )
        ...
        try:
            ...
            menu_items = (
                ...
                pystray.MenuItem(
                    "Enable Mobile API",
                    self.start_server,
                    enabled=self.is_server_stopped,
                ),
                pystray.MenuItem(
                    "Disable Mobile API",
                    self.stop_server,
    ...
    def _update_linux_menu_sensitivity(self):
        if not self.use_linux_native:
            return
        is_running = self.is_server_running()
        if self.gtk_item_start:
            self.gtk_item_start.set_sensitive(not is_running)
        if self.gtk_item_stop:
            self.gtk_item_stop.set_sensitive(is_running)

    def _linux_open_web_ui(self, widget):
        self.open_web_ui()

    def _linux_show_status(self, widget):
        self.show_server_status()

    def _linux_start_server(self, widget):
        self.start_server()

    def _linux_stop_server(self, widget):
        self.stop_server()

    def _linux_restart_server(self, widget):
        self.restart_server()

    def _linux_quit(self, widget):
