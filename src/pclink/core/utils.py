...
def add_firewall_rule(
    rule_name: str, port: int, protocol: str = "UDP", direction: str = "out"
) -> tuple[bool, str]:
...
class _LinuxStartupManager(_StartupManager):
    ...
    def _detect_packaged_installation(self) -> bool:
        ...
        try:
            ...
            if pclink_path and Path(pclink_path).resolve().parts[:3] == (
                "/",
                "usr",
                "bin",
            ):
...
"""
            ...
            subprocess.run(
                ["systemctl", "--user", "daemon-reload"], capture_output=True, env=env
            )
            subprocess.run(
                ["systemctl", "--user", "enable", svc_path.name],
                capture_output=True,
                env=env,
            )
