...
from .extension_router import mgmt_router, mount_extension_routes, runtime_router
...
def create_api_app(
    ...
    async def verify_api_key(
        ...
        # Authenticate using Device Tokens (UUIDs)
        ...
        # Fallback to Web Session (if accessed via browser)
        try:
    ...
    def verify_mobile_api_enabled():
        if not (
            ...
            raise HTTPException(
                status_code=503, detail="Mobile API is currently disabled."
            )
        return True
    ...
    try:
        ...
        async def web_ui_fallback():
            return {"message": "Web UI not available", "error": str(e)}
    ...
    app.include_router(
        terminal_router, prefix="/terminal", tags=["Terminal"], dependencies=MOBILE_API
    )
    app.include_router(
    ...
    async def change_password(payload: ChangePasswordPayload):
        ...
        if not web_auth_manager.change_password(
